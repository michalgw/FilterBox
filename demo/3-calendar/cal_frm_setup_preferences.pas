...
begin
  ...
  for i := 1 to 7 do
    ComboBox_FirstDayOfWeek.Items.AddObject(LongDayNames[i], TObject(i));
...
begin
  ...
  begin
    if MinDate <= 0 then
      PSCDateEdit_MinDate.Date := StrToDate('01' + DateSeparator + '01' + DateSeparator + '2002')
    else
      PSCDateEdit_MinDate.Date := MinDate;
    if MaxDate <= 0 then
      PSCDateEdit_MaxDate.Date := StrToDate('01' + DateSeparator + '01' + DateSeparator + '2002')
    else
...
begin
  with APSCCalendar do
  begin
    CheckBox_WorkDay1.Caption := ShortDayNames[1 + (i - 1) mod 7];
    CheckBox_WorkDay2.Caption := ShortDayNames[1 + i mod 7];
    CheckBox_WorkDay3.Caption := ShortDayNames[1 + (1 + i) mod 7];
    CheckBox_WorkDay4.Caption := ShortDayNames[1 + (2 + i) mod 7];
    CheckBox_WorkDay5.Caption := ShortDayNames[1 + (3 + i) mod 7];
    CheckBox_WorkDay6.Caption := ShortDayNames[1 + (4 + i) mod 7];
    CheckBox_WorkDay7.Caption := ShortDayNames[1 + (5 + i) mod 7];
