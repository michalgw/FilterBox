...
const
  faOnlyFiles    = faAnyFile and not (faDirectory or SysUtils.faVolumeID);
  faDirsAndFiles = faAnyFile and not (SysUtils.faVolumeID);
...
begin
  ...
  begin
    ...
    begin
      PSCModifyFileAttr( BakName, SysUtils.faReadOnly, 0 );
      DeleteFile( BakName);
    end;
        ...
        If TVarData(Value).VType = VarString Then
          Begin
            If Kind = tkEnumeration Then
              Begin
                TypInfo.SetEnumProp(Instance,PropInfo,String(Value));
                exit;
              End
            Else
              Begin
